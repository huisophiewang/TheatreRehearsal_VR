...
while mode < 17:    # sometimes we break out, needing to start at the beginning
    for i in range(len(lines)):
        if mode == 0:
            ...
            if "CODE_SIGN_IDENTITY[sdk=iphoneos*]" in lines[i]:
                lines[i] = '				"CODE_SIGN_IDENTITY[sdk=iphoneos*]" = "iPhone Distribution";'
                lines[i] += "\n"
                mode += 1

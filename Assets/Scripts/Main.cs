...
public class Main : VHMain
{
    ...
    static string[] ProfLines = Audio_Prof_Male;
    bool isCloser = true;
	
    
	public static string[] Line_Stud_Taylor = new string[] { 
        "What punishment did the Prince announce? What suffering lies in store for me?", 
        "Is the Prince's punishment any less awful than doomsday?", 
        "Ha, banishment? Be merciful and say death. Exile is much worse than death."
    };
	
	
	static string[] Audio_Stud_Max = new string[] { "coward1", "news3", "banished1" };
	
	static string[] Audio_Prof_Male = new string[] { "10_male", "11_male", "12_male", "13_male", "14_male", "15_male", "16_male", "17_male_a", "18_male" };
	
    static string[] Audio_Prof_Female = new string[] { "1_female", "2_female", "3_female", "4_female", "5_female", "6_female", "7_female", "8_female", "9_female" };
	
	string[] Dialogue = new string[] { Audio_Prof_Male[0], Audio_Stud_Max[0], "0_player", Audio_Prof_Male[1], "0_player", Audio_Stud_Max[1], "1_player", Audio_Prof_Male[2], "1_player",
                                          Audio_Prof_Male[3], "1_player",  Audio_Prof_Male[4],Audio_Stud_Max[0],"0_player", Audio_Stud_Max[1],"1_player",Audio_Stud_Max[2],"2_player",
                                          Audio_Prof_Male[5], "2_player", Audio_Prof_Male[6],"2_player", Audio_Prof_Male[7],"2_player",Audio_Prof_Male[8]};
	
    string[] Dialogue_female = new string[] {Audio_Prof_Female[0], Audio_Stud_Max[0], "0_player", "pause",
											Audio_Prof_Female[1], "0_player", Audio_Stud_Max[1], "1_player", "pause",
											Audio_Prof_Female[2], "1_player", 
											Audio_Prof_Female[3], "1_player", "pause",
											Audio_Prof_Female[4], Audio_Stud_Max[0], "0_player", Audio_Stud_Max[1], "1_player", Audio_Stud_Max[2], "2_player", 
                                            Audio_Prof_Female[5], "2_player", "pause",
											Audio_Prof_Female[6], "2_player", "pause",
											Audio_Prof_Female[7], "2_player", "pause",
											Audio_Prof_Female[8], "2_player"};
	...
	// address of the parallel port in decimal
	private int parallel_port_addr = 888;
    ...
    public override void Start()
    {
        ...
        {
            ProfName = FeProfName;
            ProfLines = Audio_Prof_Female;
			Dialogue = Dialogue_female;
        }
		
		// parallel port address on current PC
		//parallel_port_addr = 53240;
		parallel_port_addr = 61432;  //0xEFF8
    }
    ...
    public void Update()
    {
        ...
        timer += Time.deltaTime;  // Time.deltaTime is the time difference between two frames
		string minutes = Mathf.Floor(timer / 60).ToString("00");
		//string seconds = Mathf.Floor(timer % 60).ToString("00");
		string seconds = (timer % 60).ToString("00.00");
		
		OVRDebugInfo.clock_text = "Total Time spent " + minutes + ":" + seconds;
		...
		//&& timer > threshold
        if (Input.GetKeyDown(KeyCode.Space) && (!m_sbm.GetCharacterVoice(ProfName).isPlaying) && (!m_sbm.GetCharacterVoice("Maarten").isPlaying)
		...
		// Game is active
        {
			...
			Debug.Log(string.Format("Current Progress, Step: {0}, Time : {1}", index, DateTime.Now));
			
			//reset timer
            // timer = 0.0f;
			OVRDebugInfo.showClock = true;
			
            string[] items = Dialogue[index].Split('_');

            if (items.Length > 1 && String.Compare(items[1], "player", true) == 0)
			// Player's turn
            {	
			
                int player_line_index = Int32.Parse(items[0]);
                string UI_PlayerLine = Line_Stud_Taylor[player_line_index];
				
				OVRDebugInfo.player_line_text = UI_PlayerLine;
				OVRDebugInfo.showPlayerLine = true;
				
				SendNumToPort(2);

            }
            else if (items.Length == 1 && String.Compare(items[0], "pause", true) == 0)
			{
				// pause at least 1 second 
				OVRDebugInfo.showPlayerLine = false;
			}
			else
            // Not player's turn
            {
		...
		// Idle or Game ends
		{
            ...
            if (index == Dialogue.Length)
            {
                ...
                OVRDebugInfo.player_line_text = "Sorry, you have failed the rehearsal.";
				OVRDebugInfo.showPlayerLine = true;
	...
	// added by Sophie, for sending a number to parallel port
	public void SendNumToPort(int num)
	{
		PortAccess.Output(parallel_port_addr, 0);
		PortAccess.Output(parallel_port_addr, num);
		// maintain for 10 milliseconds
		System.Threading.Thread.Sleep(10);
		PortAccess.Output(parallel_port_addr, 0);
		Debug.Log(string.Format("Just sent number {0} to port, Time:{1}", num, DateTime.Now));
	}
}

...
public class SpeechBox_Web : MonoBehaviour
{
    ...
    {
        TypingEnabled = false;
        string url = string.Format("http://vhtoolkitweb/VHMsgAsp/VHMsgSite.aspx?SpeechUserId={0}&UserMessage={1}&ClientNeedsResponse=true&ParticipantId={2}", speechUserId, message.Replace(" ", "%20"), m_Main.ParticipantId);
        WWW www = new WWW(url);
        Debug.Log(url);
        yield return www;
